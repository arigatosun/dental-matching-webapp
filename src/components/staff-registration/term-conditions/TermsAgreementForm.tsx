...
interface TermsAgreementFormProps {
  onComplete: (data: { agreed: boolean }) => void;
}
...
export const TermsAgreementForm: React.FC<TermsAgreementFormProps> = ({ onComplete }) => {
  ...
  const handleSubmit = () => {
    if (agreed) {
      ...
      onComplete({ agreed });
      router.push('/register/staff/completed'); // 登録完了ページへのパス
    }
  };
