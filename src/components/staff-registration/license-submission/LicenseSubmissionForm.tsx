...
interface LicenseSubmissionFormProps {
  onNext: (data: any) => void;
}

export const LicenseSubmissionForm: React.FC<LicenseSubmissionFormProps> = ({ onNext }) => {
  ...
  const handleSubmit = () => {
    if (frontFile && backFile) {
      ...
      console.log('Uploading front license file:', frontFile);
      console.log('Uploading back license file:', backFile);
    ...
    <Container maxWidth="md">
      <Grid container spacing={3}>
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>国家資格免許証（表面）</Typography>
              {frontPreview ? (
                <CardMedia
                  component="img"
                  image={frontPreview}
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>国家資格免許証（裏面）</Typography>
              {backPreview ? (
                <CardMedia
                  component="img"
                  image={backPreview}
